...
lemma semantics_constraints_eq_all {f} [ZKField f]
  ...
  | cons hd tl ih =>
      rcases hd with ⟨c, d⟩
      cases hsc : semantics_zkexpr c witness ram_values with
      | none =>
          simp [semantics_constraints, constraintOk, hsc]
      | some cf =>
          cases hsd : semantics_zkexpr d witness ram_values with
          | none =>
              simp [semantics_constraints, constraintOk, hsc, hsd]
          | some df =>
              by_cases hEq : cf == df
              · simp [semantics_constraints, constraintOk, hsc, hsd, hEq, ih]
              · simp [semantics_constraints, constraintOk, hsc, hsd, hEq]

lemma perm_all_eq {α} (p : α → Bool) {l₁ l₂ : List α} (h : List.Perm l₁ l₂) :
    l₁.all p = l₂.all p := by
  induction h with
  | nil => rfl
  | cons _ _ ih => simp [List.all, ih]
  | swap a b l =>
...
lemma semantics_constraints_append {f} [ZKField f]
  ...
  | cons hd tl ih =>
      ...
      cases hsc : semantics_zkexpr c witness ram_values with
      | none =>
          simp [semantics_constraints, hsc]
      | some cf =>
          cases hsd : semantics_zkexpr d witness ram_values with
          | none =>
              simp [semantics_constraints, hsc, hsd]
          | some df =>
              by_cases hEq : cf == df
              · simp [semantics_constraints, hsc, hsd, hEq, ih]
              · simp [semantics_constraints, hsc, hsd, hEq]
